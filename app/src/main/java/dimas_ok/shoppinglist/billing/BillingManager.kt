package dimas_ok.shoppinglist.billing
            ...
            if (!purchase.isAcknowledged) {
                ...
                bClient?.acknowledgePurchase(acParams) {
                    if (it.responseCode == BillingClient.BillingResponseCode.OK) {
                        savePref(true)
                        Toast.makeText(activity, "Дякую за придбання!", Toast.LENGTH_LONG).show()
                    } else {
                        savePref(false)
                        Toast.makeText(activity, "Не вдалось здійснити покупку!", Toast.LENGTH_LONG).show()
                    }
                }
